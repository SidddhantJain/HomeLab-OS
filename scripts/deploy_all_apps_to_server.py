#!/usr/bin/env python3
"""
HomeLab OS — Complete Multi-Application Remote Deployment Suite
Transfers and hosts FiscalFlow, MadeNMore, Stitch 3D Studio, and HomeLab OS onto media-server@192.168.0.182.
Creates desktop launchers, configures systemd background services, and opens ports for 0.0.0.0 LAN/Internet access.
"""
...
LOCAL_PROJECTS = {
    "FiscalFlow": r"D:\Siddhant\projects\FiscalFlow",
    "MadeNMore": r"D:\Siddhant\projects\MadeNMore",
    "Stitch3DStudio": r"D:\down1\stitch_madenmore_3d_studio_website"
}

def create_project_zip(project_name, source_dir):
    temp_zip = os.path.join(tempfile.gettempdir(), f"{project_name}.zip")
    if os.path.exists(temp_zip):
        os.remove(temp_zip)

    print(f"📦 Packaging '{project_name}' from '{source_dir}' -> '{temp_zip}'...", flush=True)
    skip_dirs = {"node_modules", ".venv", "venv", ".git", ".pytest_cache", "__pycache__", "dist", "build"}
...
def deploy():
    print("=" * 70)
    print(f" 🚀 HomeLab OS Multi-Application Remote Deployment to {USER}@{HOST}")
    print("=" * 70)
    ...
    try:
        ...
        print(" [OK] Connected to target server via SSH!")
    ...
    # Step 1: Prepare directories on remote server
    run_ssh_cmd(client, f"mkdir -p {REMOTE_HOMELAB_DIR} {REMOTE_APPS_DIR} ~/Desktop ~/.local/share/applications")

    # Step 2: Package and Upload HomeLab OS Core
    root_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    homelab_zip = create_project_zip("HomeLabOS", root_dir)
    remote_homelab_zip = "/tmp/HomeLabOS.zip"
    upload_file_sftp(sftp, homelab_zip, remote_homelab_zip)

    run_ssh_cmd(client, f"unzip -o {remote_homelab_zip} -d {REMOTE_HOMELAB_DIR} && rm -f {remote_homelab_zip}")

    # Step 3: Package and Upload External Applications (FiscalFlow, MadeNMore, Stitch3DStudio)
    for app_name, local_path in LOCAL_PROJECTS.items():
        if not os.path.exists(local_path):
            print(f" ⚠️ Warning: Local path '{local_path}' not found. Skipping {app_name}.")
            continue

        app_zip = create_project_zip(app_name, local_path)
        remote_app_zip = f"/tmp/{app_name}.zip"
        target_app_dir = f"{REMOTE_APPS_DIR}/{app_name}"

        upload_file_sftp(sftp, app_zip, remote_app_zip)
        run_ssh_cmd(client, f"mkdir -p {target_app_dir} && unzip -o {remote_app_zip} -d {target_app_dir} && rm -f {remote_app_zip}")

    # Step 4: Install System Dependencies & Python Venvs
    print("\n[Step 4/6] Setting up System Packages & Python Virtual Environments...")
    run_ssh_cmd(client, f"echo '{PASS}' | sudo -S apt update && sudo -S apt install -y python3-pip python3-venv python3-full git nodejs npm unzip curl xdg-utils")
    ...
    run_ssh_cmd(client, f"cd {REMOTE_HOMELAB_DIR}/frontend && npm install || true")

    # FiscalFlow Venv
    run_ssh_cmd(client, f"cd {REMOTE_APPS_DIR}/FiscalFlow/backend && python3 -m venv .venv && .venv/bin/pip install --upgrade pip && .venv/bin/pip install fastapi uvicorn pydantic sqlite3-api || .venv/bin/pip install fastapi uvicorn pydantic")
    ...
    print("\n[Step 5/6] Launching All Application Services on 0.0.0.0...")
    ...
    # 4. MadeNMore Web App (Port 5174)
    run_ssh_cmd(client, f"cd {REMOTE_APPS_DIR}/MadeNMore && (nohup npx vite --host 0.0.0.0 --port 5174 </dev/null >../madenmore.log 2>&1 &)")
    ...
    print("\n[Step 6/6] Installing Executable Desktop Launchers on Server Desktop (`/home/server/Desktop/`)...")
...
""",
        "MadeNMore.desktop": f"""[Desktop Entry]
Version=1.0
Type=Application
...
Comment=Launch all 4 hosted web services in web browser
...
"""
    ...
    for file_name, content in shortcuts.items():
        ...
        print(f"   [OK] Installed shortcut: ~/Desktop/{file_name}")

    # Mark trusted in GNOME desktop if available
    ...
    print("\n[Verification] Checking Live Server Ports on 192.168.0.182...")
